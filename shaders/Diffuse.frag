...
void main() {
  ...
  vec3 v_position3 = v_position.xyz / v_position[3];
  vec3 l = normalize(u_light_pos - v_position3);
  float r = length(u_light_pos - v_position3);
  out_color = vec4(1, 1, 1, 0) * ((vec4(u_light_intensity, 1) / (r * r)) * max(0, dot(v_normal, vec4(l, 1))));
  out_color.a = 1;
}
